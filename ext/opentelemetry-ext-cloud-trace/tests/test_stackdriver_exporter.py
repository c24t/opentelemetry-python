...
from opentelemetry.ext.cloud_trace import CloudTraceSpanExporter, _extract_status, _extract_attributes, _extract_events, \
    _extract_links
...
class TestStackdriverSpanExporter(unittest.TestCase):
    def setUp(self):
        ...
        self.extracted_attributes_variety_pack = {"attribute_map": {
            "str_key": AttributeValue(string_value={
                "value": "str_value",
                "truncated_byte_count": 0,
            }),
            "bool_key": AttributeValue(bool_value=False),
            "double_key": AttributeValue(string_value={
                "value": "1.421",
                "truncated_byte_count": 0,
            }),
    ...
    def test_extract_status(self):
        ...
        self.assertEqual(_extract_status(Status(canonical_code=StatusCanonicalCode.OK)), {"details": None, "code": 0})
        self.assertEqual(_extract_status(Status(canonical_code=StatusCanonicalCode.UNKNOWN, description="error_desc")),
                         {"details": None, "code": 2, "message": "error_desc"})

    def test_extract_attributes(self):
        self.assertEqual(_extract_attributes({}), {"attribute_map": {}})
        self.assertEqual(_extract_attributes(self.attributes_variety_pack), self.extracted_attributes_variety_pack)
        # Test ignoring attributes with illegal value type
        self.assertEqual(_extract_attributes({
            "illegal_attribute_value": dict()
        }), {"attribute_map": {}})
    ...
    def test_extract_events(self):
        ...
        event1 = Event(name="event1", attributes=self.attributes_variety_pack, timestamp=time_in_ns)
        event2 = Event(name="event2", attributes={'illegal_attr_value': dict()}, timestamp=time_in_ns2)
        self.assertEqual(_extract_events([event1, event2]),
                         {"time_event": [
                             {"time": time_in_ms_and_ns,
                              "annotation": {"description": {"value": "event1", "truncated_byte_count": 0},
                                             "attributes": self.extracted_attributes_variety_pack}
                              },
                             {"time": time_in_ms_and_ns2,
                              "annotation": {"description": {"value": "event2", "truncated_byte_count": 0},
    ...
    def test_extract_links(self):
        ...
        trace_id = "6e0c63257de34c92bf9efcd03927272e"
        span_id = "95bb5edabd45950f"
        link1 = Link(context=SpanContext(
            trace_id=int(trace_id, 16), span_id=int(span_id, 16),
            is_remote=False
        ), attributes={})
        link2 = Link(context=SpanContext(
            trace_id=int(trace_id, 16), span_id=int(span_id, 16),
            is_remote=False
        ), attributes=self.attributes_variety_pack)
        link3 = Link(context=SpanContext(
            trace_id=int(trace_id, 16), span_id=int(span_id, 16),
            is_remote=False
        ), attributes={
        ...
        self.assertEqual(_extract_links([link1, link2, link3]), {"link": [
            {"trace_id": "6e0c63257de34c92bf9efcd03927272e", "span_id": "95bb5edabd45950f", "type": "CHILD_LINKED_SPAN",
             "attributes": {"attribute_map": {}}},
            {"trace_id": "6e0c63257de34c92bf9efcd03927272e", "span_id": "95bb5edabd45950f", "type": "CHILD_LINKED_SPAN",
             "attributes": self.extracted_attributes_variety_pack},
            {"trace_id": "6e0c63257de34c92bf9efcd03927272e", "span_id": "95bb5edabd45950f", "type": "CHILD_LINKED_SPAN",
